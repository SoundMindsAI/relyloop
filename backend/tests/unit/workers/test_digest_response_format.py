...
def test_schema_caps_suggested_followups_at_five() -> None:
    ...
    # feat_digest_executable_followups Story 2.1: items are structured
    # {kind, rationale, search_space_json} objects (NOT strings).
    # feat_digest_executable_followups_swap_template Story 2.1 (Tier B):
    # adds the uniform template_id string field to every item +
    # widens kind enum to 4 values (narrow / widen / text / swap_template).
    # OpenAI strict mode rejects oneOf/if/then on item subschemas, so the
    # worker drops the empty-string sentinel before Pydantic dispatch
    # per spec D-29 instead of conditionally requiring template_id.
    items = sf["items"]
