...
"""Integration test for I-5 (feat_query_normalizer_typed_pipeline Story 1.4).

Seeds a study whose ``query_normalizer`` is a typed ``normalizer_pipeline``
(not a Phase-1 bundle), drives one trial through the real ``run_trial``
worker with a REAL ``ElasticAdapter.render`` (only ``search_batch`` is
stubbed), and asserts:

  * the persisted ``trials.params['query_normalizer']`` is one of the
    pipeline's powerset labels (the worker passed it through opaquely), and
  * the native query body the adapter rendered reflects
    ``normalize_pipeline(query_text, steps_for_label(chosen))`` — proving the
    trial → adapter → normalize_pipeline runtime path Story 1.4 completes,
    including for non-bundle labels.
...
_TEMPLATE_BODY = '{"query": {"match": {"title": "{{ query_text }}"}}}'

# A pipeline whose powerset includes non-bundle labels like
# "lowercase+strip_punctuation" that the old bundle-only hook could not apply.
_PIPELINE_STEPS = [
...
async def _seed_pipeline_study() -> TrialFixture:
    ...
    async with factory() as db:
        ...
        # Casing + punctuation so strip_punctuation/lowercase are observable.
        query = await repo.create_query(
            db,
            id=str(uuid.uuid4()),
            query_set_id=query_set.id,
            query_text="Hello, BEST!",
            ...
            search_space={
                "params": {
                    "query_normalizer": {
                        ...
                        "steps": [s.value for s in _PIPELINE_STEPS],
                    }
                }
            },
...
def _ask_pipeline_trial(storage, optuna_study_name: str) -> tuple[int, str]:
    """Simulate the orchestrator: ask() + suggest_categorical over powerset labels."""
    config = {"max_trials": 100, "sampler": "tpe"}
    ...
    trial = study.ask()
    chosen = trial.suggest_categorical("query_normalizer", _pipeline_labels(_PIPELINE_STEPS))
    return trial.number, str(chosen)


async def test_trial_runner_records_and_applies_pipeline_label(
    monkeypatch: pytest.MonkeyPatch,
    _cluster_credentials,
) -> None:
    fixture = await _seed_pipeline_study()
    storage = build_storage(get_settings().database_url)
    trial_number, chosen = _ask_pipeline_trial(storage, fixture.optuna_study_name)
    ...
    # The worker passed query_normalizer through opaquely; it persisted a label.
    factory = get_session_factory()
    async with factory() as db:
        trials = await repo.list_trials_for_study(db, fixture.study_id)
    assert len(trials) == 1
    persisted = trials[0].params
    assert persisted["query_normalizer"] in _pipeline_labels(_PIPELINE_STEPS)
    assert persisted["query_normalizer"] == chosen

    # I-5: the rendered native body reflects normalize_pipeline for the chosen
    # label — including a non-bundle label the old bundle-only hook would reject.
    assert rendered_bodies, "adapter.render was never called"
    title_slot = rendered_bodies[0]["query"]["match"]["title"]
    assert title_slot == normalize_pipeline("Hello, BEST!", steps_for_label(chosen))
