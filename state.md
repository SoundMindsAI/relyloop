...
**Last updated:** 2026-05-12 (after `feat_github_pr_worker` merged via PR #45)

---

## Current branch / execution context

- **Branch:** `main` — `feat_github_pr_worker` merged via PR #45 (squash commit `a80433b`); finalization happening on `docs/finalize-feat-github-pr-worker`.
- **Active feature:** none in flight. Next-up: `feat_github_webhook` (will mirror the per-repo `webhook_secret_ref` pattern that `feat_github_pr_worker` established for `auth_ref`).
- **Alembic head:** `0005_digests` (no migrations in `feat_github_pr_worker` — additive on existing `proposals` + `config_repos` columns).
- **Coverage:** above the 80% gate. 453 unit tests passing locally + 2 xpassed (pre-existing capability_check flake) + Epic 3 contract test for the new feature.
...
2. **`feat_studies_ui`** — UI shell + `/studies` + `/studies/[id]`. Also lands the digest panel that consumes `GET /api/v1/studies/{id}/digest` shipped by `feat_digest_proposal`.
3. **`feat_chat_agent`** — streaming chat orchestrator. Will consume `POST /api/v1/proposals/{id}/open_pr` for the agent-tool "open this PR" path.
4. **`feat_proposals_ui`** — `/proposals` review surface. Consumes the proposal CRUD endpoints `feat_digest_proposal` shipped + the `POST /proposals/{id}/open_pr` endpoint shipped by `feat_github_pr_worker`.
