...
- **Headliners (idea-stage):** `feat_fts_rank_ordering`, `feat_chat_last_message_preview`, `feat_overnight_autopilot`, `feat_query_normalization_tuning`, `feat_study_convergence_indicator`, `feat_ubi_llm_study_comparison` (side-by-side UBI-vs-LLM study comparison view + deferred cluster-detail rung badge, P2 — Phase 2 split out of `feat_demo_ubi_study_comparison`).
