...
- **Branch:** `feat/ubi-judgments`. `feat_ubi_judgments` shipped end-to-end (11 of 13 stories: migration + domain + reader + readiness service + dispatcher + 2 endpoints + worker + agent tool + frontend dialog + value-delta cards + runbook/FAQ/data-model docs). Stories 5.2 (E2E suite) + the 7-of-10 Story 5.1 doc artifacts deferred to `chore_ubi_e2e_suite` + `chore_ubi_docs_followup`; DB-backed integration tests deferred to `chore_ubi_integration_tests`. All three follow-up idea files committed.
- **Active feature:** `feat_ubi_judgments` PR pending push.
...
- `feat_ubi_judgments` (PR pending push). Ships the UBI substrate + API + worker + frontend + operator docs. Three follow-up idea files committed for the deferred sub-scope (E2E, doc completeness, DB-backed integration tests).
