...
**Last updated:** 2026-05-11 (after `feat_study_lifecycle` Phase 2 merged via PR #25)

---

## Current branch / execution context

- **Branch:** `main` — clean. `feat_study_lifecycle` Phase 2 merged via PR #25 (squash commit `25bb5c9`).
- **Active feature:** none in flight. Next-up: `feat_llm_judgments` (judgment-list LLM rubric runner) per the Queued list below.
- **Alembic head:** unchanged at `0003_study_lifecycle_schema` — Phase 2 added zero migrations.
- **Coverage:** above the 80% gate. 297 unit tests + 2 xpassed (capability-check flake) + ~50 integration + 19 contract + Phase 2's new layers.
...
- **`backend/app/eval/qrels_loader.py` is an MVP1 stub.** It raises `JudgmentsTableMissing` because the `judgments` child table (owned by `feat_llm_judgments`) hasn't shipped. Integration tests monkeypatch the loader; production `feat_study_lifecycle` Phase 2 orchestrators will hit this stub the first time a real study runs against a real cluster — `feat_llm_judgments` MUST ship before any study is dispatched in production. The real implementation lands atomically with that feature.
