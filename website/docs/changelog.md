...
| Release | Theme | Adds |
|---|---|---|
| **MVP1 / `v0.1.0`** (shipped) | The Loop | ES + OpenSearch adapter, OpenAI-compatible LLM, GitHub provider, single-tenant, Optuna/TPE Bayesian loop, Git-PR apply path, conversational agent |
| **MVP2 / `v0.2`** | Three-Engine + Real Signals | Apache Solr adapter + UBI-derived judgments (incl. hybrid UBI + LLM) |
| **MVP3 / `v0.3`** | Observable | Self-hosted observability (Langfuse + SigNoz), audit log, lineage, PII redaction |
| **GA / `v1.0`** | Production-ready | Orchestrator hardening, full error model, 90% coverage, security gates — no new product surface |
...
the current tag and its full notes. `v0.1.0` (MVP1) is the first public
release.
