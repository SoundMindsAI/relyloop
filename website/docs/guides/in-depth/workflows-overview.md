<!-- GENERATED by website/scripts/build_guides.py from docs/08_guides/workflows-overview.md — DO NOT EDIT. -->
...
- **Reference:** [`docs/03_runbooks/local-dev.md`](https://github.com/SoundMindsAI/relyloop/blob/main/docs/03_runbooks/local-dev.md).
...
- **Reference:** [`docs/01_architecture/llm-orchestration.md`](https://github.com/SoundMindsAI/relyloop/blob/main/docs/01_architecture/llm-orchestration.md).
...
- **How (UI):** [`/clusters`](https://github.com/SoundMindsAI/relyloop/blob/main/ui/src/app/clusters/page.tsx) → "Register cluster" modal. Provide name, engine type (`elasticsearch` or `opensearch`), URL, auth kind, credentials reference.
...
- **How (bulk):** `make seed-clusters` runs [`scripts/seed_clusters.py`](https://github.com/SoundMindsAI/relyloop/blob/main/backend/app/scripts/seed_clusters.py) to register `local-es` + `local-opensearch` for the tutorial.
...
- **Reference:** [`docs/04_security/github-token-handling.md`](https://github.com/SoundMindsAI/relyloop/blob/main/docs/04_security/github-token-handling.md) for PAT scoping + rotation.
...
- **How:** `make seed-es` loads 1,000 sample products from [`samples/products.json`](https://github.com/SoundMindsAI/relyloop/blob/main/samples/products.json) into the `products` index.
...
- **How (UI):** [`/templates`](https://github.com/SoundMindsAI/relyloop/blob/main/ui/src/app/templates/page.tsx) → "Create template". Provide name, engine type, Jinja2 template body, and a `declared_params` map (parameter name → type).
...
- **Versioning:** Templates are immutable. To evolve a template, **fork it to v2** ([`/templates/[id]`](https://github.com/SoundMindsAI/relyloop/blob/main/ui/src/app/templates/[id]/page.tsx) → "Fork to v{N+1}" button) — this preserves study lineage (`study.query_template_id` always points at a specific version).
...
- **How (UI):** [`/query-sets`](https://github.com/SoundMindsAI/relyloop/blob/main/ui/src/app/query-sets/page.tsx) → "Create query set" modal. Bind to a cluster (the queries are evaluated against that cluster's index). Optionally upload a CSV in the same modal.
...
- **How (UI):** [`/query-sets/[id]`](https://github.com/SoundMindsAI/relyloop/blob/main/ui/src/app/query-sets/[id]/page.tsx) → `<QueriesTable>` with inline Edit / Metadata / Delete icon-buttons per row.
...
- **How (UI):** [`/query-sets/[id]`](https://github.com/SoundMindsAI/relyloop/blob/main/ui/src/app/query-sets/[id]/page.tsx) → "Generate judgments" modal. Pick the cluster, template, and rubric.
...
- **How (UI):** [`/query-sets/[id]`](https://github.com/SoundMindsAI/relyloop/blob/main/ui/src/app/query-sets/[id]/page.tsx) → "Generate judgments" modal → **Method** picker: `UBI (click-through)`, `UBI (dwell-time)`, or `Hybrid UBI + LLM` (UBI for high-signal pairs, LLM fills the long tail). The detail page then shows a value-delta card comparing the UBI ratings against any prior LLM list.
...
- **Runbook:** [`../03_runbooks/ubi-judgment-generation.md`](https://github.com/SoundMindsAI/relyloop/blob/main/docs/03_runbooks/ubi-judgment-generation.md) for per-engine UBI capture setup + converter selection.
...
- **How (UI):** [`/judgments/[id]`](https://github.com/SoundMindsAI/relyloop/blob/main/ui/src/app/judgments/[id]/page.tsx) — table with a source filter (`llm` / `human` / `click`), inline override.
...
- **How (UI):** [`/judgments/[id]`](https://github.com/SoundMindsAI/relyloop/blob/main/ui/src/app/judgments/[id]/page.tsx) → "Calibrate" button.
...
- **How (UI):** [`/studies`](https://github.com/SoundMindsAI/relyloop/blob/main/ui/src/app/studies/page.tsx) → "Create study". Pick cluster + query set + template + judgment list, define the search space (per-parameter type + bounds), set `max_trials`.
...
- **How (UI):** [`/chat`](https://github.com/SoundMindsAI/relyloop/blob/main/ui/src/app/chat/page.tsx) → "New conversation". Type: *"Tune `product_search v1` against `tutorial_queries` on `local-es:products`, max 10 trials."* Watch the agent's tool calls (`get_schema`, `run_query`, `create_study`) render inline.
...
- **How (UI):** [`/studies/[id]`](https://github.com/SoundMindsAI/relyloop/blob/main/ui/src/app/studies/[id]/page.tsx) — header summary (status, baseline, best metric) + trials table (auto-polls every 3 seconds while running). Sort by primary metric, trial number, or `ended_at`.
...
- **How (UI):** [`ConfidencePanel`](https://github.com/SoundMindsAI/relyloop/blob/main/ui/src/components/studies/confidence-panel.tsx) renders on the study detail page once the study completes. Four sections:
...
- **How (API):** `GET /api/v1/studies/{id}` returns the full `confidence` shape inline; see [`backend/app/domain/study/confidence.py`](https://github.com/SoundMindsAI/relyloop/blob/main/backend/app/domain/study/confidence.py).
- **Glossary:** every `(i)` icon resolves to a definition under the `confidence.*` namespace in [`ui/src/lib/glossary.ts`](https://github.com/SoundMindsAI/relyloop/blob/main/ui/src/lib/glossary.ts).
...
- **How:** Automatic. Optuna's `RDBStorage` holds trial history in Postgres; the `on_startup` hook in [`backend/workers/all.py`](https://github.com/SoundMindsAI/relyloop/blob/main/backend/workers/all.py) sweeps running studies and enqueues `resume_study(study_id)` for each, picking up from the last completed trial.
...
- **What's in it:** Top trial config, primary metric delta vs. baseline, parameter importance scores (Optuna's FanovaImportanceEvaluator), parameter importance bar chart, narrative summary, up to 5 suggested follow-up studies. See [`feat_digest_proposal`](https://github.com/SoundMindsAI/relyloop/tree/main/docs/00_overview/implemented_features/2026_05_11_feat_digest_proposal/).
...
- **How (UI):** [`/proposals`](https://github.com/SoundMindsAI/relyloop/blob/main/ui/src/app/proposals/page.tsx) — filter by status (`pending` / `pr_opened` / `merged` / `rejected`), source (`chat_triggered` / `digest_triggered`), and cluster. Auto-refetches every 30s when any visible row has an open PR (so webhook-driven status updates are seen without manual reload).
...
- **How (UI):** [`/proposals/[id]`](https://github.com/SoundMindsAI/relyloop/blob/main/ui/src/app/proposals/[id]/page.tsx) — side-by-side YAML diff of baseline vs. proposed params (`ConfigDiffPanel`), baseline-vs-achieved metric delta, suggested-followups panel, PR panel (4-state: pending / opening / opened / errored).
...
- **How (API):** `POST /api/v1/proposals/{id}/open_pr` — returns `202 ACCEPTED`, enqueues `open_pr` worker job with a deterministic `_job_id` (Arq dedup; AC-12 from [`feat_github_pr_worker`](https://github.com/SoundMindsAI/relyloop/tree/main/docs/00_overview/implemented_features/2026_05_12_feat_github_pr_worker/)).
...
- **Reference:** [`docs/03_runbooks/pr-open-debugging.md`](https://github.com/SoundMindsAI/relyloop/blob/main/docs/03_runbooks/pr-open-debugging.md).
...
- **Reference:** [`docs/03_runbooks/webhook-debugging.md`](https://github.com/SoundMindsAI/relyloop/blob/main/docs/03_runbooks/webhook-debugging.md).
...
- **How (UI):** [`/chat`](https://github.com/SoundMindsAI/relyloop/blob/main/ui/src/app/chat/page.tsx) → conversation list shows each past chat with a preview of the last user/assistant message + the timestamp of the last activity (added in [`chore_chat_last_message_preview`](https://github.com/SoundMindsAI/relyloop/tree/main/docs/00_overview/implemented_features/2026_05_14_chore_chat_last_message_preview/)). Click to resume.
...
- **Reference:** [`docs/04_security/github-token-handling.md`](https://github.com/SoundMindsAI/relyloop/blob/main/docs/04_security/github-token-handling.md) + [`pr-open-debugging.md`](https://github.com/SoundMindsAI/relyloop/blob/main/docs/03_runbooks/pr-open-debugging.md).
...
- **How (UI):** [`/clusters/[id]`](https://github.com/SoundMindsAI/relyloop/blob/main/ui/src/app/clusters/[id]/page.tsx) action bar → "Delete" (type-the-name confirmation gate).
...
- **Read the user stories:** [`docs/02_product/mvp1-user-stories.md`](https://github.com/SoundMindsAI/relyloop/blob/main/docs/02_product/mvp1-user-stories.md) is the canonical narrative source — every workflow above traces back to a numbered US-N.
