...
function ResetDemoStateDisclosure(): React.ReactElement {
  return (
...
export function StartHereChecklist({
  ...
  // Fully-seeded state — checklist itself is hidden (operator has finished
  // first-run), but render the reset-disclosure standalone so the affordance
  // is still reachable. Per bug_demo_reseed_fake_metric_regression follow-up:
  // operators need to re-trigger the reseed (e.g., after fake-data was
  // generated by the legacy sync path) without first deleting every cluster.
  if (hasClusters && hasQuerySetsWithJudgments && hasStudies) {
    return (
      <Card data-testid="start-here-checklist-fully-seeded">
        <CardContent className="pt-6">
          <ResetDemoStateDisclosure />
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <Card data-testid="start-here-checklist">
      ...
      <CardContent>
        ...
        {/*
          Reset-to-demo-state disclosure renders in every checklist state
          (first-run, partial, and via the fully-seeded fast-path above).
          Originally gated on ``!hasClusters``; that gate hid the
          affordance once a reseed had completed, even though the
          operator may need to re-trigger the reseed (e.g., to replace
          the legacy fake-metric studies with real ones from
          bug_demo_reseed_fake_metric_regression). The disclosure is
          collapsed by default + carries a "this will WIPE everything"
          confirmation dialog, so unconditional rendering is safe.
        */}
        <ResetDemoStateDisclosure />
      </CardContent>
    </Card>
