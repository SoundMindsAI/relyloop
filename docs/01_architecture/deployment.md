...
### Local LLM (native-first + `ollama-docker` fallback)

`scripts/install.sh` reads `RELYLOOP_LLM` (`scripts/lib/relyloop_llm.sh`, allowlist `{ollama, ollama-docker}`); OFF by default. An explicit `OPENAI_BASE_URL` always wins (no native wire, no container).

- **`ollama` (native-first, `feat_bundled_llm_native_detection`).** `resolve_native_ollama` (`scripts/lib/relyloop_native_llm.sh`) probes the host (`http://localhost:11434/api/tags`, validating the Ollama `models` shape) and, if found, exports `OPENAI_BASE_URL=http://host.docker.internal:11434/v1` + the model defaults + the sentinel `openai_key`. The api/worker carry `extra_hosts: ["host.docker.internal:host-gateway"]` so this resolves on Linux (no-op on Mac/Windows; needs Docker ≥ 20.10 / Compose v2). After `up --wait`, install.sh restarts api+worker (capability-cache refresh) and runs a container-side reachability check — on Linux a `127.0.0.1`-bound Ollama passes the host probe but is unreachable from the bridge, so it warns to bind `OLLAMA_HOST=0.0.0.0`. If no native Ollama is found, install.sh prints guidance and comes up LLM-free.
- **`ollama-docker` (the shipped `feat_bundled_local_llm` container).** Appends `bundled-llm` to `COMPOSE_PROFILES` → the opt-in `ollama` Compose service (`profiles: ["bundled-llm"]`, no host port, `http://ollama:11434`, pinned `ollama/ollama` image, `qwen3.5:4b`). CPU-only on Docker-for-Mac (no Metal passthrough) → slow; the zero-install fallback. install.sh wires the in-network endpoint + sentinel and restarts api+worker after `up --wait`. The api/worker do **not** `depends_on` ollama (a cross-profile `depends_on` would break the default `up`).
