...
| `/studies/compare` | Side-by-side LLM-vs-UBI study comparison (`feat_ubi_llm_study_comparison`, 2026-06-05). `?a={id}&b={id}` → `<StudyComparisonPage>` resolves the pair via `useStudyComparePairing` (`GET /api/v1/studies/compare`), normalizes columns LLM-left / UBI-right regardless of URL order, and renders four diff panels: best-metric (kind-normalized `ubi − llm` delta, direction-aware), best-trial param table (`=`/`Δ`/em-dash), digest-narrative sentence diff (jsdiff `diffSentences`), and a **two-series** convergence overlay (the first multi-series Recharts `<LineChart>` — consumes `StudyDetail.convergence.best_so_far_curve` when present, else derives it client-side from `/studies/{id}/trials`). Non-fatal `CROSS_CLUSTER`/`TARGET_MISMATCH`/`OBJECTIVE_MISMATCH` warnings render a banner; pairing failures render a keyed error state. Read-only — no migration, no write path. Entry points self-gate on pair existence: a "Compare with the {UBI\|LLM} study" button on `/studies/{id}` (via `useStudyPair` → `GET /studies/{id}/pair`) and a "View matched study comparison" link on the UBI value-delta card (via `useJudgmentListStudy` → `GET /judgment-lists/{id}/study`). | `feat_ubi_llm_study_comparison` |
