...
**Phase 2 (deferred).** A "Reset to demo state" button + `POST /api/v1/_test/demo/reseed` endpoint is captured at [`docs/00_overview/planned_features/feat_home_first_run_demo_nudge/phase2_idea.md`](../00_overview/planned_features/feat_home_first_run_demo_nudge/phase2_idea.md). Requires refactoring the CLI seed script's `docker compose exec psql` truncate path into an asyncpg-friendly module — non-trivial scope deliberately split from the polish-layer PR.
...
When adding new tooltips on existing surfaces (Phase 2 / Phase 3 work — see [`feat_contextual_help/phase2_idea.md`](../00_overview/planned_features/feat_contextual_help/phase2_idea.md) and [`phase3_idea.md`](../00_overview/planned_features/feat_contextual_help/phase3_idea.md)), extend `glossary.ts` with new keys following the same naming convention (dotted, lowercase, `<scope>.<aggregate>` or `<scope>.<aggregate>.<wire_value>`) and add a parity-test case for any new enum group.
...
- Owning feature specs:
  - [`feat_studies_ui/feature_spec.md`](../00_overview/planned_features/feat_studies_ui/feature_spec.md) — bulk of MVP1 screens
  - [`feat_proposals_ui/feature_spec.md`](../00_overview/planned_features/feat_proposals_ui/feature_spec.md) — proposals list + detail
  - [`feat_chat_agent/feature_spec.md`](../00_overview/planned_features/feat_chat_agent/feature_spec.md) — chat surface (also covers backend agent)
