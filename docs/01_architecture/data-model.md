...
[`feat_study_convergence_indicator`](../00_overview/implemented_features/2026_05_31_feat_study_convergence_indicator/) (no migration — reads existing columns only) consumes `(optuna_trial_number, primary_metric, status, is_baseline)` via `backend/app/db/repo/trial.py::list_complete_optuna_trials_for_study` to build the convergence verdict surfaced on `StudyDetail.convergence`. The filter is `status = 'complete' AND is_baseline = FALSE AND primary_metric IS NOT NULL` pushed into SQL; the pure-domain classifier in `backend/app/domain/study/convergence.py` re-applies the same filter defensively. No schema delta.
