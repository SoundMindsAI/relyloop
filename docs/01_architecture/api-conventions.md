...
**Full-text search.** Six list endpoints accept `?q=<text>` for full-text search powered by a Postgres `tsvector GENERATED ALWAYS AS … STORED` column + GIN index per row (added by `feat_data_table_primitive` migrations `0008`–`0013`). The predicate is `search_vector @@ plainto_tsquery('english', q)`. Results are filtered (not rank-ordered — rank ordering deferred to MVP2; see [`docs/00_overview/planned_features/02_mvp2/feat_fts_rank_ordering/idea.md`](../00_overview/planned_features/feat_fts_rank_ordering_mvp2/idea.md)) so the existing `(created_at, id)` cursor stays correct. The Zod schema in the UI rejects under-length input (min 2 chars) before the backend ever sees the call.
