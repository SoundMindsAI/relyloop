...
|---|---|---|
| **MVP1 / v0.1** | "The Loop" | ES + OpenSearch adapter (single `ElasticAdapter`), OpenAI direct (`openai` SDK + function calling), GitHub Git provider, single-tenant (no `tenants` table, no `tenant_id`), no auth, basic structured logging, Docker Compose, Apache 2.0 LICENSE, 80% backend coverage gate. **No** observability stack, **no** audit_log, **no** lineage, **no** Fusion, **no** SSO, **no** API keys. |
