...
| **Engine support** | Elasticsearch 8.11+/9.x + OpenSearch 2.x/3.x today; **Apache Solr planned MVP2**; one adapter, one workflow | OpenSearch only | OpenSearch only | Solr (since day one) + ES + OpenSearch | Solr + ES | Solr (primary) + OpenSearch (partial) | ES only | Solr + ES |
