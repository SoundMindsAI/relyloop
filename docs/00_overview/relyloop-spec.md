...
- `generate_judgments_from_ubi(query_set_id, cluster_id, target, since, until?, converter, llm_fill_threshold?)` → `JudgmentList` — *(MVP2, requires the engine to emit the UBI schema — OpenSearch UBI plugin, Elasticsearch o19s fork, or — on Solr — a UBI-capture component the operator installs; stock Solr ships none, so RelyLoop's local demo synthesizes the events)* read `ubi_queries` + `ubi_events`, aggregate per-(query, doc) features via `UbiReader`, run the named `SignalsConverter`, and (optionally) fill the long tail with LLM-as-judge when impression count < `llm_fill_threshold`. Emits a judgment list with mixed `source` rows (`click` + optional `llm`). See §14.
