...
- [x] 🟡 **Seed `good first issue` tickets** — done; 8 curated issues created (#355–#362), each grounded in a verified idea file or confirmed `file:line` with a problem statement, pointers, and a definition-of-done. 7 are `good first issue` (arq `aclose`, UBI click-bucket render, contract allowlists, arq spy fixture, baseline-phase test isolation, credential-restart doc, spec Solr-label/link fix); #362 (E2E teardown FK ordering) is `help wanted` as a notch-harder task. Created `tests` label. Stopped at 8 high-confidence tickets rather than padding to the plan's 10–15 — more can be mined from the `02_mvp2/` idea backlog later. Note: #361 overlaps the spec wording in PR #354 — coordinate/rebase.
