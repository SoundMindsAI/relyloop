...
| # | Priority | Feature | Type | One-liner | Depends on | Status |
|---|---|---|---|---|---|---|
| 1 | P1 | [bug_healthz_degraded_blocks_ui_engine_subset](planned_features/02_mvp2/bug_healthz_degraded_blocks_ui_engine_subset/idea.md) | Bug | `/healthz` returns HTTP **503** whenever the `elasticsearch` or `opensearch` subsystem probe reports `unreachable` — even when the operator **intentionally** excluded that engine via `RELYLOOP_ENGINES | — | Idea — bug captured during live operator use of the engine-subset feature |
