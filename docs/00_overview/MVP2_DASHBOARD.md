...
_Reflects feature-folder state as of **2026-06-17** (latest mtime of any planned/implemented feature `.md` file). Regenerated by `make dashboard` and the `mvp1-dashboard-regen` pre-commit hook. For the rich local view (filter chips, type colors), open [`mvp2_dashboard.html`](mvp2_dashboard.html) in a browser._
...
| Specced features done | **27 / 29** (93%) — of features *past the idea stage* (those with a spec); the idea backlog below is NOT in this denominator, so 100% ≠ release complete |
...
| # | Priority | Feature | Type | One-liner | Depends on | Status |
|---|---|---|---|---|---|---|
| 1 | P2 | [feat_engine_version_selection](planned_features/02_mvp2/feat_engine_version_selection/idea.md) | Feature | Phase 1 ships engine *selection* — operator picks which engines start at install time. Engine *versions* stay hardcoded in [`docker-compose.yml`](../../docker-compose.yml) at `elasticsearch:9.4.1`, `o | — | Idea — spun out of the shipped [`feat_selective_engine_startup_and_demo`](../../../implemented_features/2026_06_17_feat_selective_engine_startup_and_demo/feature_spec.md) (was its deferred Phase 2; split into its own folder at finalization per operator request) |
