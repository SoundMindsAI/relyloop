...
| # | Priority | Feature | Type | One-liner | Depends on | Status |
|---|---|---|---|---|---|---|
| 1 | P1 | [bug_llm_capability_cache_no_refresh](planned_features/02_mvp2/bug_llm_capability_cache_no_refresh/idea.md) | Bug | The OpenAI capability check runs **exactly once**, as a fire-and-forget task in the FastAPI `lifespan` startup hook ([`backend/app/main.py:94`](../backend/app/main.py) — `run_capability_check_backgrou | — | Idea — surfaced live while debugging a demo-reseed failure (operator's stack had been up 34h) |
