...
**Date:** 2026-05-29
**Status:** Idea — scoped to MVP2 (moved from `00_unsure/` 2026-05-29). The core capability is small and fits the existing parameter model. One **gating design fork remains** — the prod-reproducibility question (below) — which `/spec-gen` must resolve; if no clean deployment path exists, this idea may be deferred out of MVP2 at spec time. Tracked as a release item rather than parked because the operator has a clear MVP2 intent and the theme fits the "Real Signals" / relevance-quality story.
**Priority:** P2 — a scope-respecting differentiator in the "query understanding" space no competitor's *optimizer* touches. Lower than the MVP2 anchors (Solr, UBI); gated on the spec-time design fork below.
...
A small **pre-render hook** applies the selected normalizer to `query_text` *before* the template interpolates it (`query_text` is the implicit render param — [`template_validator.py:58`](../../../../../backend/app/domain/study/template_validator.py#L58), declared via the module-level `_IMPLICIT_PARAMS` frozenset). The Optuna loop then *discovers* whether a normalization rule helps the judgment set, the same way it discovers the best boost — and the winning normalizer travels in the proposal/PR like any other parameter.
...
1. **Prod reproducibility** — how does the chosen normalizer get deployed in the operator's actual query pipeline? Options: (a) the apply-path PR carries a normalizer declaration the operator's search service reads; (b) the proposal body documents the required normalizer and the operator replicates it manually; (c) scope to only normalizers the engine itself can apply at query time (narrow). This decision determines whether the feature is viable at all.
   **Recommended default for /spec-gen:** **Option (b) — documentation hand-off in the proposal body** for MVP2. The proposal/PR body gets a new "Operator-side requirement" section that names the winning normalizer and the rule string, and includes a copy-pasteable Python/JS snippet implementing it. The merge contract is: "RelyLoop measured X gain against this normalizer; you must replicate it in your query layer for production parity." Option (a) is the more powerful path but materially expands apply-path scope (proposals today carry only engine-side parameters); defer to a follow-on idea (`feat_apply_path_normalizer_declaration`) if MVP2 adoption shows the manual replication is friction. Option (c) is too restrictive — it would exclude contraction expansion, which is the operator's headline ask.
2. **Normalizer library scope** — which rules ship? Lowercasing and trim are safe and engine-symmetric-ish; contraction expansion needs a dictionary (English-only? operator-supplied?); spell-correction is probably too far (drifts toward the analyzer/index-time boundary and needs a corpus).
   **Recommended default:** Ship **four built-in normalizers** for MVP2: `none` (passthrough), `lowercase`, `lowercase+trim`, `lowercase+trim+expand_contractions` (English-only, ~30-entry static dictionary in `backend/app/domain/study/normalizers.py`). Spell-correction is out of scope — it crosses into corpus-aware territory. Operator-supplied contraction dictionaries are out of scope for MVP2; capture as a deferred phase if requested.
3. **Overlap with analyzers** — lowercasing at query-time may *duplicate or conflict* with a lowercase token filter already in the index analyzer (double-normalization is usually harmless but worth validating). The capability probe / schema read could warn when a chosen normalizer is redundant with the field's analyzer.
   **Recommended default:** Out of scope for the tunable mechanism — double-normalization is harmless. Add a non-blocking advisory line to the study-detail "tuned parameters" panel when the winning normalizer applies a transform the field's analyzer already does. The capability probe is *not* extended; the advisory is computed from `get_schema` data the adapter already returns.
4. **Is categorical the right shape, or should it be a typed sub-object** (an ordered list of normalization steps)? Categorical keeps it inside the existing search-space model with zero schema change; a sub-object is more expressive but more work.
   **Recommended default:** **Categorical** with named bundles (per Q2's recommended default) for MVP2 — zero schema change, fits the existing `CategoricalParam` model, easy to lift into a typed sub-object later. A typed sub-object that lets operators compose arbitrary step sequences is more expressive but introduces a new search-space type and forces a migration; defer as a Phase 2 idea if the four built-in bundles prove insufficient.
