...
- [x] **Story 2.1** — `enqueue_followup_study` Arq job (FR-3, FR-5, FR-6, FR-7 worker side, FR-9 events 1-7). Commit `9718812a`. Backend `make lint` + `make typecheck` + full `make test-unit` (1197 pass) all green. 7 integration tests written at `backend/tests/integration/test_auto_followup.py` (skip on host since Postgres isn't reachable from the `.venv`; CI runs against service containers). `backend/tests/unit/test_workers.py` extended to include `enqueue_followup_study` in the expected WorkerSettings.functions set.
- [x] **Story 2.2** — Digest worker trigger (FR-1 trigger). Commit `70f61d8c`. 53-line trigger block inserted at the end of `generate_digest` (after `digest_complete` log, before `finally`). Uses deterministic `_job_id=f"enqueue_followup_study:{study_id}"` per layer-1 idempotency (D-11). 5 source-inspection tests at `backend/tests/unit/workers/test_digest_followup_trigger.py` lock the trigger shape (condition uses `is not None` not `> 0`; deterministic _job_id; failure events use `digest_followup_*` prefix). Full `make test-unit` 1202 pass.
- [x] **Story 2.3** — Cancel cascade endpoint + Children endpoint + Telemetry event #8. Cancel handler extended with `?cascade=<bool>` (default true) routing through `cancel_study_with_chain_cascade` from Story 1.3; AC-9 wire contract (409 on terminal-parent + cascade=false) preserved by routing to plain `cancel_study` when `cascade=False`. New `GET /studies/{id}/children` endpoint returns `StudyListResponse(data=[StudySummary], next_cursor=None, has_more=False)`. Custom `_parse_cascade` dependency emits `INVALID_CASCADE_PARAM` (400) on invalid values (per spec §8.5). 18 router-level tests at `backend/tests/unit/api/test_studies_router_chain_endpoints.py` verify endpoint registration + cascade param parsing. Telemetry event #8 (`auto_followup_cancelled_with_parent`) already wired in Story 1.3 service. Full `make test-unit` 1220 pass.
- [x] **Epic 2 phase gate** — `make lint` ✓, `make typecheck` ✓ (409 files clean), `make test-unit` ✓ (1220 pass). Cumulative-diff GPT-5.5 cross-model review run on the Epic 1+2 backend diff (~2832 lines across 18 files):
  - **F5 (Medium, accepted)** — budget gate's unknown-model fallback was less conservative than the plan (treated unknown pricing as `0.0` max_call_cost). Fixed in `backend/workers/auto_followup.py`: now emits `auto_followup_skipped_budget` with `reason="unknown_model_pricing"` and refuses to enqueue, mirroring the `digest.py:543` pattern.
  - **F3 (Medium, accepted)** — `cancel_study_with_chain_cascade(cascade=False)` was silently returning terminal parents unchanged, diverging from its docstring. Fixed: now delegates to `cancel_study` so terminal parents raise `InvalidStateTransition` (preserves AC-9 single-cancel contract). `test_cascade_no_cascade_on_terminal_parent_raises` updated.
  - **F4 (Medium, accepted as doc-fix)** — sort key in `compute_first_decile_max` is `optuna_trial_number` (not `created_at` as the plan said) because `Trial` has no `created_at` column. The implementation docstring already explains why; treating this as a spec/plan correction rather than an impl change.
  - **F1 (Medium, accepted as doc-fix)** — `5.5` test case is impossible with the current `int | None` field (Pydantic rejects fractional floats before the validator runs). Drop `5.5` from the plan's invalid-cases list; the `{-1, 6, 100}` parametrize covers the meaningful range.
  - **F6 + F7 + F8 (Medium, partial-accept)** — Story 2.1/2.2/2.3 integration + contract test extensions for the cascade endpoint, the digest trigger runtime, and the depth-3 chain are CI-gated (need real Postgres + Redis). The host can't run them; the router-introspection + source-inspection tests cover the regression surface that matters for local development. The full integration matrix runs against service containers on the PR.
  - **F2 (High, deferred per D-11)** — the layer-2 idempotency race window is acknowledged in the plan; captured as `chore_auto_followup_parent_advisory_lock` for MVP2 hardening. **NEW finding:** cascade-cancelling a *completed* parent can race with a pending followup-enqueue Arq job (parent stays `completed`, gate passes, child created after cascade returns). Captured as `chore_auto_followup_completed_parent_stop_chain_race` idea file. Race window is small (~Arq poll cycle); impact is one extra child the operator can cancel manually.
- [ ] Story 3.1 — Glossary entries + Auto-followup chain panel component (FR-10)
- [x] **Story 3.1** — Glossary entries + Auto-followup chain panel component (FR-10). Commit `d640b593` (bundled with 3.2 + 3.3 + Node 22 pin since the changes interlock at the same files). 4 new glossary keys; `<AutoFollowupChainPanel>` + `useStudyChildren` hook; wired into `/studies/[id]`. 7 vitest cases in `auto-followup-chain-panel.test.tsx`.
- [x] **Story 3.2** — Wizard depth selector (FR-11). Commit `d640b593`. `AUTO_FOLLOWUP_DEPTH_WIZARD_VALUES = [0,1,2,3,4,5]` in `enums.ts` with the source-of-truth comment per CLAUDE.md "Enumerated Value Contract Discipline"; wizard-0 is the off sentinel that maps to undefined on submit (wire-0 reserved for worker decrement per D-12).
- [x] **Story 3.3** — Cancel modal cascade radio (FR-8 frontend). Commit `d640b593`. `chainChildren` prop on `StudyActionBar` (named NOT `children` per cycle-2 C2-4); native `<input type="radio">` (no new dep needed); `useCancelStudy` extended with `CancelStudyVars{cascade?}`; 6 vitest cases in `study-action-bar-cascade.test.tsx`. E2E Playwright spec deferred — the agent environment cannot run Playwright; backend integration tests cover the cascade contract.
- [x] **Epic 3 phase gate** — `pnpm typecheck` ✓, `pnpm lint` ✓ (0 errors, 105 pre-existing warnings), `pnpm test` ✓ (744 pass, +13 new), `pnpm build` ✓. All on Node 22.
- [x] **Story 4.1** — Documentation updates. New runbook at [`docs/03_runbooks/auto-followup-debugging.md`](../../../03_runbooks/auto-followup-debugging.md) (8 FR-9 events + 4 auxiliary events catalog, 6 quick diagnostic recipes, manual-mitigation steps for runaway chains). `state.md` updated with the feature entry at the top of "Most recent meaningful changes."
- [x] **Post-implementation** — test coverage audit (planned test files all exist; integration tests are CI-gated per the documented host-side gap); deferred-work sweep (no deferred phases per spec §3 single-phase delivery); tangential observations sweep (one new idea captured during the Epic 1+2 phase-gate review: `chore_auto_followup_completed_parent_stop_chain_race/idea.md`); guide impact assessment (touches `/studies/[id]` page and create-study modal — operator-facing tutorial guide for chains is a separate `chore_guide_auto_followup_walkthrough` opportunity not yet captured because the chain itself is opt-in and most operators won't encounter it until they read the runbook).
- [ ] **PR push + CI watch + Gemini adjudication + final GPT-5.5 review + finalize** — the next pipeline turn (push, open PR, monitor CI, adjudicate Gemini, run final cross-model review, then move to `implemented_features/`).
