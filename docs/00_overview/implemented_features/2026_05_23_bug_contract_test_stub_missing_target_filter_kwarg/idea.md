...
**Priority:** P2 — anyone running the in-container `pytest backend/tests/contract/` (or `make test-contract` via the Compose stack) sees `2 failed, 282 passed`. **CI on `main` is green** including the contract-test job: both tests hit the `pytest.skip("Could not register cluster — ES likely unreachable")` branch because [`_body()` at `test_error_codes.py:102-111`](../../../../backend/tests/contract/test_error_codes.py#L102-L111) hardcodes `base_url: "http://elasticsearch:9200"` (Compose-network DNS) but [GHA's `pr.yml` `backend` job at `.github/workflows/pr.yml:85-135`](../../../../.github/workflows/pr.yml) binds the Elasticsearch service container at `localhost:9200`. The cluster registration's verification step can't resolve the hostname → returns non-201 → `pytest.skip` fires → the broken stub is never exercised. Locally (via `make up`) the dev-deps container shares the Compose network, so `http://elasticsearch:9200` resolves, the cluster registers, and the TypeError surfaces. The test as written is broken (its actual assertion is unreachable past the TypeError) — the local failure is correctly flagged regardless of CI behavior. Repro verified via the canonical in-container pattern as of 2026-05-23: `2 failed, 2 warnings in 2.79s` with `TypeError: _Stub.list_targets() got an unexpected keyword argument 'target_filter'`.
...
- Composes with [`feat_orchestrator_zero_streak_abort`](../../../00_overview/implemented_features/2026_05_22_feat_orchestrator_zero_streak_abort/feature_spec.md) (shipped 2026-05-22, PR #191) only in the sense that running its phase gate exposed the pre-existing bug.
