...
- Manual `make dashboard` invocation is out of scope. The standard pre-commit hook (`mvp1-dashboard-regen`) WILL fire automatically on each commit that touches feature folders and will refresh `MVP1_DASHBOARD.md` + `mvp1_dashboard.html` + `DASHBOARD.md` + `dashboard.html`; those auto-regenerated files land in PR A as downstream-of-pre-commit changes (not chore-authored content). No story should call `make dashboard` directly.
...
- **Do not** invoke `make dashboard` manually as part of this chore's task list — the regen is driven by the standard pre-commit hook (`mvp1-dashboard-regen`) on any commit that touches a feature folder's metadata, which IS expected to fire automatically here. The chore's deliverable is the 4 doc edits + folder move; the dashboard regen diff that lands in PR A is downstream-of-pre-commit, not chore-authored content. Manually running `make dashboard` would couple the chore to an unrelated cadence and create churn.
...
- **No other in-flight PR is editing either target file.** Status: confirmed by `git log --since="2026-05-23" -- docs/00_overview/planned_features/01_mvp1/chore_e2e_seed_acme_helper_dead/idea.md ui/tests/e2e/helpers/coverage-audit.md` (no commits since the OBE). Risk: rebase conflicts if a parallel PR edits the same regions — low for both files at this time.
...
- Then file line 4 of [`docs/00_overview/planned_features/01_mvp1/chore_e2e_seed_acme_helper_dead/idea.md`](../chore_e2e_seed_acme_helper_dead/idea.md) **MUST** start with the literal prefix `**Status:**` AND contain all three required signal substrings:
  - Example values:
  ...
  - And running `git diff --stat HEAD~1 -- docs/00_overview/planned_features/01_mvp1/chore_e2e_seed_acme_helper_dead/idea.md` **MUST** show exactly 1 file changed with 1 insertion and 1 deletion (the single line-4 swap).
...
- `docs/02_product/`: this chore *is* the doc update. Specifically:
  - Update `docs/00_overview/planned_features/01_mvp1/chore_e2e_seed_acme_helper_dead/idea.md` (FR-1).
...
| FR-5 | AC-5 | Epic 2 / Story 2.1 — Post-merge: `git mv` folder to `implemented_features/` + push finalization commit | n/a (post-merge) | none |
...
- **D-3 (2026-05-25):** No manual `make dashboard` invocation in any story. Rationale: the dashboard regen is two-tracked — the pre-commit hook (`mvp1-dashboard-regen`) fires automatically on any commit that touches a feature folder, AND `make dashboard` is a separate operator-cadence target. The pre-commit hook will already refresh the dashboard on each chore commit (this is unavoidable given CLAUDE.md absolute rule 7 forbids `--no-verify`); coupling a manual `make dashboard` step to chore tasks would just create unrelated diff churn. The auto-regenerated dashboard files in PR A are accepted-as-downstream, not chore-authored. (Patched in cycle-1 cross-model review on the plan: GPT-5.5 Pass A High finding flagged the original wording as contradicting the inescapable pre-commit-hook behavior.)
