...
1. **Top-level `try/except BaseException` barrier** wraps the entire `run_demo_reseed` body. On exception, writes `status="failed"` with `started_at` + `finished_at` + `failed_reason=f"{type(exc).__name__}: {str(exc)[:200]}"` to Redis, then **re-raises**. Cites: idea.md §"Proposed capabilities" #1 (locks re-raise-after-status-write — preserves Arq's `JobExecutionFailed` ops record AND worker-log traceback). The existing inner handler at lines 150-173 keeps its `return` (no re-raise) because retrying the destructive wipe is the wrong behavior for `reseed_demo_state` failures.
