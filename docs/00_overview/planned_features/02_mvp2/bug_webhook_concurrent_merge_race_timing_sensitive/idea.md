...
> **Static-read correction 2026-06-05 (P2 backlog grooming) — the production write-site appears race-SAFE; this is more likely a test-harness fragility than a production data-correctness bug.** A close read of the actual pointer-update function [`config_repo.py:152-177`](../../../../../backend/app/db/repo/config_repo.py) (`update_config_repo_last_merged_pointer`, the single FR-2 write site) shows it does the right thing:
> 1. `SELECT … FOR UPDATE` on the `config_repos` row (serializes concurrent merges on the same repo).
> 2. Re-reads the currently-tracked proposal's `pr_merged_at` **under the lock** (`current_ts_stmt`).
> 3. Writes only if `pr_merged_at_new > current_ts`.
>
> Because steps 2-3 run while holding the row lock, the *second* merge to acquire the lock sees the *first's* committed pointer and compares against it — the newer-timestamp proposal wins regardless of arrival order. That is the textbook race-free compare-under-lock pattern, **not** the broken "read row → compare in Python without a lock → UPDATE" pattern the original hypothesis below describes. The webhook receiver path ([`github.py:194-213`](../../../../../backend/app/api/webhooks/github.py)) also dedups duplicate/out-of-order deliveries via `mark_proposal_pr_merged` returning `None`.
>
> **What's actually left:** the reported `test_ac7` failure under a second lifespan task is therefore most likely a **test-harness artifact** (the warmup task contending on the same event loop / connection pool during the test), gated today by `RELYLOOP_DISABLE_STARTUP_WARMUP=1`. **Pickup should START by running the test to confirm whether any real defect exists** before touching the (apparently-correct) merge handler — do not assume the production race from the hypothesis below. If the repro is purely a test-pool contention issue, this becomes a small test-isolation fix, not a production-correctness fix.
