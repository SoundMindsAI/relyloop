...
- Version selection at install time (`ES_IMAGE_TAG`/`OS_IMAGE_TAG`/`SOLR_IMAGE_TAG` env vars, curated version matrix, install.sh version flags). **Tracked:** [phase2_idea.md](phase2_idea.md).
- Version display in the reset-to-demo modal. ES/OS have no version-report path today; adding one for the reset modal alone is not justified. **Tracked:** [phase2_idea.md](phase2_idea.md).
- SSE migration of the reseed status endpoint. The existing 2s poll already streams step-by-step progress; defer SSE until measured insufficiency. **Tracked:** [phase3_idea.md](phase3_idea.md).
