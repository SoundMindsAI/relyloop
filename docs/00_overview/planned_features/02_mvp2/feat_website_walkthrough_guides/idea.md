...
- **10 screenshot walkthrough decks** in [`ui/public/guides/<NN_slug>/`](../../../../../ui/public/guides/) — each with a `metadata.json` (title, description, estimated_time, tags, `screenshots[]` of `{file, caption}`, optional `video`), ~48 numbered PNGs total, and a `walkthrough.webm` slow-motion video.
...
- Keep the hand-written `nav:` block (no nav plugin) and add a CI freshness gate that re-runs the generator and fails on `git status --porcelain` drift — same pattern as the existing copy-docs gate. `--strict` forbids orphan pages, so generator output and nav must stay in lockstep.
...
P2: high-value for evaluation/onboarding but not blocking any in-flight MVP2 work, and the underlying content is already complete and maintained — this is a surfacing/pipeline task, not new product. Filed in `02_mvp2/` as the active-release bucket. Largest open question for the spec stage is asset strategy (commit ~12–13 MB of PNG/WebM/MP4 vs generate-uncommitted-in-CI) and whether the freshness gate should include MP4 (requires ffmpeg on that runner) or treat it as a manually-regenerated artifact.
