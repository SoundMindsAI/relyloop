...
- Verification (engine-backed headroom guards the enriched scenarios — GPT-5.5 cycle 2 F2 + plan-cycle-1 F4):
  1. **Engine-backed headroom test, parameterized over all 5 scenarios** (integration; deterministic — no optimizer): render each scenario's template with its **default/baseline** param values and with a **known-better** param set, evaluate NDCG@10 against the authored docs+judgments via the existing eval engine, and assert `0.40 ≤ baseline ≤ 0.70`, `better − baseline ≥ 0.10`, and `better < 0.99`. **CI coverage scope:** the 4 ES/OpenSearch scenarios are **hard CI gates** (backend CI runs ES + OS service containers); the Solr scenario **skip-gates in CI** (the backend CI lane has no Solr container per `infra_solr_ci_readiness`) — it runs locally when Solr is up and is covered by the manual operator-path (bullet 3), mirroring the established demo-reseed engine tolerance (`is_engine_reachable`). This is the honest Solr-CI posture, not a coverage gap claim.
...
- 2026-06-02 — **D-18** (GPT-5.5 cycle 2 F2; amended plan-cycle-1 F4): enriched scenarios are guarded by a deterministic **engine-backed headroom test** (baseline-vs-better-param NDCG@10 bounds, no optimizer). **CI scope:** the 4 ES/OS scenarios are hard CI gates; the Solr scenario skip-gates in CI (no Solr container in the backend CI lane per `infra_solr_ci_readiness`) and is covered by the manual operator-path — consistent with the repo's demo-reseed engine tolerance. The full 50-trial study run is exercised end-to-end for one representative scenario (`@pytest.mark.slow`) + manually for all 5 at release gate. This avoids 5×50-trial CI cost while keeping FR-5 a CI gate for the engines CI can run.
