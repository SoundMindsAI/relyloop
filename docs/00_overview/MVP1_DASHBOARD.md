...
|---|---|---|---|---|---|---|
| 1 | P2 | [bug_demo_clusters_unreachable_in_healthz](../02_product/planned_features/bug_demo_clusters_unreachable_in_healthz/feature_spec.md) | Bug | **After** the warmup task completes (typically within ~5 seconds of API startup, bounded by per-cluster `httpx` probe latency), `/healthz` reports the accurate `healthy` / `unreachable` aggregate for  | — | [PR #234](https://github.com/SoundMindsAI/relyloop/pull/234) |
