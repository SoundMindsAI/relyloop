...
| 4 | `build-guides-freshness` | own file (`build-guides-freshness.yml`) — runs on every PR with no `paths-ignore` filter (own-workflow escape from `pr.yml`'s `website/**` + `docs/**` paths-ignore so website-only PRs still get the check) | `ui/public/guides/<slug>/*` + `docs/08_guides/*.md` → `website/docs/guides/**` + `website/docs/assets/guides/**` + the `website/mkdocs.yml` managed nav fragment | `python website/scripts/build_guides.py` (stdlib-only; prunes the 3 generator-owned dirs; default invocation never shells out to ffmpeg — MP4s are best-effort and **excluded** from the gate's `git status` scope per spec D-2). The workflow also runs the generator unit tests + `mkdocs build --strict` so a website-only PR's broken nav/link is caught before merge | `scripts/ci/test_verify_build_guides_fresh.sh` exercises clean / source-drift / `git rm --cached` / brand-new-source-deck cases against a disposable `mktemp` git fixture |

The single canonical fix command for ALL four gates:
