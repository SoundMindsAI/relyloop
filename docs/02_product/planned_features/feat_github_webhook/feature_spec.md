...
- The system **MUST** define `reconcile_pr_state` as an Arq cron job registered via `WorkerSettings.cron_jobs = [cron(reconcile_pr_state, **_poll_cron_kwargs())]` in `backend/workers/all.py`. The default cadence (`relyloop_pr_poll_minutes=15`) yields `cron(..., minute={0, 15, 30, 45})`; sub-hour cadences use `minute=` only; multi-hour cadences (`relyloop_pr_poll_minutes ∈ {120, 180, 240, 360, 720, 1440}`) use `hour=set(range(0, 24, n//60)), minute={0}`. The poll-cadence override is sourced from `Settings.relyloop_pr_poll_minutes` (env var `RELYLOOP_PR_POLL_MINUTES`, default `15`); the cron-job tuple is constructed at module-load time from that value, and unsupported values fall back to the 15-minute default with a WARN log (see FR-4 whitelist).
...
- **Settings field** added to `backend/app/core/settings.py`:
  - `relyloop_pr_poll_minutes: int = Field(default=15, description="Cron cadence for the reconcile_pr_state worker. MVP1 default 15; operators can raise for low-PR-volume installs to reduce GitHub API spend.")` plus a `@field_validator` that rejects values outside the whitelist `{1, 2, 3, 4, 5, 6, 10, 12, 15, 20, 30, 60, 120, 180, 240, 360, 720, 1440}` — every divisor of 60 (sub-hour cadence) plus every multiple of 60 up to 1440 that divides 1440 (multi-hour cadence). Earlier draft used `ge=1, le=1440` without a whitelist; that allowed values like 90 or 100 that no `arq.cron(...)` invocation can express, so the worker would silently fall back to the 15-minute default (see 2026-05-12 Decision log entry).
- **`.env.example`** updated with the new env var + a sentence-long comment naming the supported values.
...
- 2026-05-12 (plan-review cycle) — **`relyloop_pr_poll_minutes` constrained to a whitelist of 18 supported values** (every divisor of 60 plus every multiple of 60 that divides 1440). Earlier `ge=1, le=1440` allowed values like 90/100 that no `arq.cron(...)` invocation can express; the worker would have silently fallen back to the 15-minute default. The whitelist makes the supported cadences explicit and gives operators a clear validation error for unsupported values. Surfaced by the implementation-plan cross-model review (Opus + GPT-5.5) on 2026-05-12.
