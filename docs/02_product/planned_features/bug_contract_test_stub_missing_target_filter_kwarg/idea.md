...
**Priority:** P2 — anyone running the in-container `pytest backend/tests/contract/` (or `make test-contract` via the Compose stack) sees `2 failed, 282 passed`. **CI on `main` is currently green** including the contract-test job, so the failures are local-environment-only: the failing tests register a real cluster via `POST /api/v1/clusters` and then monkeypatch `acquire_adapter` with a stub that no longer matches the Protocol signature; CI may be skipping these tests via the `pytest.skip("Could not register cluster — ES likely unreachable")` branch when the GHA service container is in a different state at test time, but the precise reason CI passes warrants investigation as part of this fix. Either way, the test as written is broken (its actual assertion is unreachable past the TypeError), so the local failure is correctly flagged regardless of CI behavior.
