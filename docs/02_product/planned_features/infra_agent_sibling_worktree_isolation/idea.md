...
- The bind-mount anchor pitfall: the shared Docker stack's bind mounts (verified in [`docker-compose.yml`](../../../../docker-compose.yml) at the `api` service line 76 `./migrations:/app/migrations`, line 77 `./alembic.ini:/app/alembic.ini:ro`, the `worker` service line 119–125 with the same migrations + alembic mounts plus `./samples:/app/samples:ro`, and the postgres / redis `./data/postgres` + `./data/redis` mounts at lines 28, 40) all point to the operator's main worktree, NOT the sibling. **Do not use `docker cp` or `docker exec` + redirection to write files into bind-mounted paths.**
