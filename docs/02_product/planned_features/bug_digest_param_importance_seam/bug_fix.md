...
1. **Add `scikit-learn>=1.4` as a direct dependency** in `pyproject.toml` `dependencies`. Cites: spec feature_spec.md §123 calls `get_param_importances` (Fanova default); Optuna's `[optional]` extra is too broad; PedAnova is experimental per Optuna docs and would change importance semantics. Floor `>=1.4` matches the repo's convention of floor-pinning (`optuna>=3.6`, `openai>=1.55`).
