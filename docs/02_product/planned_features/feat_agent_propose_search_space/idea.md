...
**Live-callers note (corrected 2026-05-21, spec-gen audit)**: `compute_default_params` IS used in app code — `backend/workers/digest.py:65,774` (digest narrative generation) and `backend/workers/judgments.py:52,189` (judgment generation) both import + call it to pick template defaults at render time. The earlier preflight claim that it was dead is wrong. The spec for THIS feature should NOT extend or refactor `compute_default_params`. The two functions remain side-by-side: `compute_default_params` picks *single concrete values* per render-time call; the new code picks *ParamSpec ranges* for Optuna sampling.
...
4. **Don't refactor `compute_default_params`.** Live side-by-side; cross-reference in docstrings. (Correction: `compute_default_params` is *not* dead — it has two live callers in the workers. No cleanup chore is warranted.)
