...
- **Phase 1 audit trail can now complete:** both deferred phases have been split out to standalone folders as of 2026-05-24 (Phase 2 → [`../feat_digest_executable_followups_swap_template/`](../feat_digest_executable_followups_swap_template/idea.md); Phase 3 → [`../backlog_feat_digest_template_edit_followups/`](../backlog_feat_digest_template_edit_followups/idea.md)). No `phase*_idea.md` files remain in this folder, so per `impl-execute` Step 8 it's eligible to move to `docs/00_overview/implemented_features/2026_05_24_feat_digest_executable_followups/` whenever the operator runs the finalization step.
