...
**Date:** 2026-05-14
**Status:** Deferred — Phase 1 ([`feature_spec.md`](feature_spec.md)) covers create-study modal + study-detail surface only. Phase 2 picks up the judgments-review + proposals surfaces.
**Origin:** Carved out of [`idea.md`](idea.md) §"Proposed capabilities → Phase 2 — judgments + proposals" during cycle 1 of spec-gen scope-lock (2026-05-14, idea Locked Decisions §1).
...
- Status badge — `InfoTooltip` (adjacent icon, Pattern A per [`feature_spec.md` FR-7 precedent](feature_spec.md)) showing the lifecycle and current-state meaning.
...
- `pr-panel.tsx` `Open PR…` button — `InfoTooltip asChild` (Pattern B per [`feature_spec.md` FR-9 precedent](feature_spec.md)). Glossary key: `proposal.open_pr_button`. **This is a structurally different button** from the Phase 1 digest-panel Open PR — same label, different component, different route.
...
- [`feat_contextual_help/feature_spec.md`](feature_spec.md) — Phase 1 ships the primitives, wrappers, and glossary file; Phase 2 is a per-surface application pass on top.
