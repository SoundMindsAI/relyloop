...
- **Backend:** orchestrator change (~30 LOC) — new `_last_n_all_zero` helper alongside `_last_n_all_failed`; new `_ZERO_STREAK_THRESHOLD = 20` module constant alongside `_CONSECUTIVE_FAILURE_THRESHOLD`; new `STUDY_NO_SIGNAL` error code in the error catalog; new failed_reason text. Tests: 3 integration.
...
- **Estimated size:** small — ~30 LOC orchestrator + ~3 LOC error code + ~100 LOC of integration tests. 45–60 minutes.
