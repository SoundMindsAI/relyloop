...
- [`docker-compose.yml`](../../../../docker-compose.yml) — canonical env-var → mount-path source-of-truth. Direct grep + service-boundary verification (Gemini Code Assist PR #257 review correctly flagged that my initial attribution had service names inverted):
  - `POSTGRES_PASSWORD_FILE: /run/secrets/postgres_password` at line 26 (postgres init), 69 (migrate), 96 (api), 154 (worker).
  - `CLUSTER_CREDENTIALS_FILE: /run/secrets/cluster_credentials` at line 102 (api), 160 (worker). The api service DOES mount cluster_credentials (early-draft assumption to the contrary was wrong — verified by reading service block boundaries: `api:` starts at line 80, `worker:` starts at line 135, so line 102 is inside api's env block).
