...
- **E2E tests** (`ui/tests/e2e/`):
  - **Updated post-spec, after user pushed back on deferring follow-up chores:** this feature also ships the **first Playwright infrastructure in the repo** (`@playwright/test` dev dep, `ui/playwright.config.ts`, `ui/tests/e2e/helpers/seed.ts`) plus 5 real-backend specs at `ui/tests/e2e/query_set_detail.spec.ts` covering: list+total, inline edit, metadata clear, delete-no-judgments, delete-with-judgments-409 + action-link navigation. Wired into the CI smoke-test job's existing live stack (no new job — piggybacks on the already-running `make up` stack to avoid duplicating setup). The spec §5 "no new external dependencies" rule is overridden by explicit user instruction for this PR.
