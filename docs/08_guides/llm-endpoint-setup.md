...
Native non-OpenAI provider SDKs are in the [backlog](../00_overview/relyloop-spec.md#27-phased-delivery) — the unblocking pattern (LiteLLM proxy or OpenRouter) covers most adopters today, so this is an ergonomics + cost-attribution upgrade rather than a "can't talk to that LLM" gap.
...
See [`docs/01_architecture/llm-orchestration.md`](../01_architecture/llm-orchestration.md) for the orchestration layer details and [`docs/03_runbooks/local-dev.md`](../03_runbooks/local-dev.md) for the broader local-dev runbook.
...
- [Umbrella spec §15 LLM orchestration & observability](../00_overview/relyloop-spec.md) — design rationale
- [CLAUDE.md Absolute Rule #2](../../CLAUDE.md) — secrets-via-mounted-files contract
- [CLAUDE.md Absolute Rule #8](../../CLAUDE.md) — never hardcode model names; always read from `Settings.openai_model`
